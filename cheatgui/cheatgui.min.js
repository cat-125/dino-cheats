...
bind(obj,param){(this.onChange||this.onInput||this.onClick)((_,val)=>obj[param]=val);return this;}}
...
class Input extends Widget{constructor(name='',text='',callback=null){super('div');this.addClass('cgui-input-wrapper');this.inputRef=createElem('input');this.inputRef.classList.add('cgui-input');this.ref.appendChild(this.inputRef);this.nameRef=createElem('div');this.nameRef.classList.add('cgui-input-name');this.ref.appendChild(this.nameRef);this.setText(text);this.setName(name);if(callback)this.onInput(callback);}
...
class NumberInput extends Widget{constructor(name='',value=0,callback=null){super('div');this.addClass('cgui-input-wrapper');this.inputRef=createElem('input');this.inputRef.classList.add('cgui-input');this.inputRef.type='number';this.ref.appendChild(this.inputRef);this.nameRef=createElem('div');this.nameRef.classList.add('cgui-input-name');this.ref.appendChild(this.nameRef);this.setValue(value);this.setName(name);if(callback)this.onInput(callback);}
setName(name){this.nameRef.innerHTML=name;return this;}
...
setValue(value){value=parseFloat(clamp(snap(value,this.step),this.min,this.max).toFixed(this.accuracy));this.value=value;const displayValue=value*(100/(this.max-this.min))-this.min;this.thumbRef.style.marginLeft=displayValue+'%';this.thumbRef.style.transform=`translateX(-${displayValue}%)`;this.thumbRef.textContent=value;return this;}
...
class Switch extends Widget{constructor(text='',checked=false,callback=null){super('label');const id=this.id=generateId(16);this.ref.for=id;this.addClass('cgui-switch');this.inputRef=createElem('input');this.inputRef.type='checkbox';this.inputRef.id=id;this.ref.appendChild(this.inputRef);this.inputRef.checked=checked;this.sliderRef=createElem('span');this.sliderRef.className='cgui-switch-slider';this.ref.appendChild(this.sliderRef);this.textRef=createElem('span');this.textRef.className='cgui-switch-text';this.textRef.for=id;this.ref.appendChild(this.textRef);this.ref.tabIndex=0;this.setText(text);if(callback)this.onChange(callback);}
...
return{GUIElement,View,Window,Element,Text,Button,Input,NumberInput,Slider,Switch,Tree,openPopupMenu,utils,isMobile};})();if(typeof module!=='undefined'&&typeof module.exports=='object')module.exports=cheatgui;if(typeof globalThis!=='undefined')globalThis.cheatgui=cheatgui;